...
<div class="row px-5 py-4">
  ...
  <div class="col-12 pt-4">
    ...
    @endif
    <a href="{{route('system.transaction.'.$transaction->status == 'CANCELLED' ? 'pending' : strtolower($transaction->status))}}" class="btn btn-light float-right mt-2">Return to Transaction list</a>
    @if(Auth::user()->type == "processor")
      @if(in_array($transaction->status, ['PENDING', 'ONGOING']) AND $transaction->transaction_status == "COMPLETED")
